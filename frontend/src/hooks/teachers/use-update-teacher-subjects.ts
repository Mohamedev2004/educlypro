...
/**
 * Hook for replacing a teacher's full subject assignment, from the teacher
 ...
 * Layer: Hooks
 */
export function useUpdateTeacherSubjects(t: (key: string) => string) {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: ({ id, subjectIds }: { id: number; subjectIds: number[] }) =>
      TeachersService.updateSubjects(id, { subject_ids: subjectIds }),
    onSuccess: (data) => {

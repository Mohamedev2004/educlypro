    ...
    variants: {
      variant: {
        ...
        sidebar:
          ...
          data-[active=true]:active:bg-sidebar-primary \
          data-[active=true]:active:text-sidebar-primary-foreground \
          hover:bg-sidebar-primary/30 \
          active:bg-sidebar-primary/30",
        outline:
...
function SidebarMenuBadge({
  ...
  return (
    <div
      ...
      className={cn(
        "pointer-events-none absolute end-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium tabular-nums select-none transition-colors duration-200",

        // 👉 DEFAULT (inactive)
        "text-foreground bg-transparent",

        // 👉 HOVER (only when NOT active)
        "peer-hover/menu-button:data-[active=false]:text-foreground",

        // 👉 ACTIVE (always white)
        "peer-data-active/menu-button:text-primary-foreground",

        // 👉 ACTIVE + HOVER (still white, explicit safety)
        "peer-data-active/menu-button:hover:text-white",
